...
Thread::Thread( SDL_ThreadFunction pf, const char* strThreadName, void* pData, bool isAwait )
	...
	HG_LOG->Info( SDL_LOG_CATEGORY_SYSTEM, std::format( "Thread({}) =>> {}", strThreadName, isAwait ? "AWAIT" : "ASYNC" ).c_str() );
	...
	if( isAwait ) {
		...
		HG_LOG->Info( SDL_LOG_CATEGORY_SYSTEM, strLogInfoFinished.c_str() );
	} else {
